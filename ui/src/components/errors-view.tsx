...
export function ErrorsView({ serviceFilter }: ErrorsViewProps) {
   ...
   const filteredGroups = getFilteredGroups(serviceFilter, errorsByService)
   const totalErrors = getTotalErrorCount(filteredGroups)

   if (filteredGroups.length === 0) {
      return <EmptyState serviceFilter={serviceFilter} />
   }
   ...
   return (
      <div className="h-full flex flex-col">
         ...
         <ErrorCardGrid filteredGroups={filteredGroups} />
      </div>
   )
}

const getFilteredGroups = (serviceFilter: string | null, errorsByService?: ErrorByService[]) =>
   serviceFilter
      ? (errorsByService?.filter(g => g.service === serviceFilter) ?? [])
      : (errorsByService ?? [])

const getTotalErrorCount = (filteredGroups: ErrorByService[]) =>
   filteredGroups.reduce((sum, g) => sum + g.count, 0)

function ErrorCardGrid({ filteredGroups }: { filteredGroups: ErrorByService[] }) {
...
function EmptyState({ serviceFilter }: ErrorsViewProps) {
   return (
