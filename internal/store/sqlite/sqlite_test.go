...
func TestAppendSpan(t *testing.T) {
	...
	if got.Service != "test-service" {
		t.Errorf("expected service test-service, got %s", got.Service)
	}
	if got.Status != "OK" {
...
func TestAppendSpanWithParentSpanID(t *testing.T) {
	t.Parallel()
	s := setupTestDB(t)
...
func TestGetTraceSpansEmpty(t *testing.T) {
	t.Parallel()
	s := setupTestDB(t)
...
func TestGetTracesFromSpans(t *testing.T) {
	t.Parallel()
	s := setupTestDB(t)
